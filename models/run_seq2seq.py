...
class DataTrainingArguments:
    ...
    no_save: bool = field(default=False, metadata={
        "help": "Whether to save the model in output_dir or not."},
    )
    separate_preds_by_train: bool = field(default=False, metadata={
        "help": "Whether to print separate {preds,results} files based on the name of the training set."},
    )
    get_confidence: bool = field(default=False, metadata={
        "help": "Whether to print outputs with confidence scores."}
    )
...
def main():
    ...
    if data_args.get_confidence:
        # override Seq2SeqTrainer with our implementation that returns "confidence"-esque scores
    ...
    if training_args.do_eval:
        ...
        if data_args.separate_preds_by_train:
            basename = os.path.basename(data_args.validation_file).replace(".json", "") + \
                       "_" + os.path.basename(data_args.train_file).replace(".json", "") + \
                       "_seed" + str(training_args.seed)
        else:
            basename = os.path.basename(data_args.validation_file).replace(".json", "")
        ...
        if trainer.is_world_process_zero():
            with open(output_pred_file, "w") as writer:
                if data_args.get_confidence:
                    for pred, confidence in zip(tokenizer.batch_decode(predictions.predictions, skip_special_tokens=True),
                                                predictions.confidence):
                        writer.write(f"{pred}\t{confidence}\n")
                else:
